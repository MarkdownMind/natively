...
describe('getOpenAiMaxOutput (issue #298)', () => {
  ...
  test('gpt-5.x and o-series keep the full requested budget at the current default', () => {
    ...
    for (const model of ['gpt-5.4', 'gpt-5.5', 'gpt-5.6-sol', 'gpt-6-astra', 'gpt-5', 'o1-mini', 'o3-mini', 'o4-mini']) {
      assert.equal(
  ...
  test('gpt-5.x caps at 128000, o-series at 100000 if a larger budget is requested', () => {
    ...
    const HUGE = 200000;
    for (const model of ['gpt-5', 'gpt-5.1', 'gpt-5.2', 'gpt-5.4', 'gpt-5.5', 'gpt-5.6-sol', 'gpt-6-astra', 'gpt-5-mini']) {
